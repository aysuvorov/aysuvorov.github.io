...
1. В общем случае, для каждой экспериментальной единицы, которая имеет координаты по характеристикам ($x$, $y$) на контурной матрице, мы вычисляем расстояние от центра матрицы до точки. Данное расстояние может быть использовано в качестве веса для данной экспериментальной единицы. 
...
2. Вновь определяются попарные комбинации характеристик. В качестве примера классификатора используем SVM with the radial basis function kernel, однако возможно использовать любой классификтор. 
