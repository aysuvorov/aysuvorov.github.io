...
1. Метод оценки корреляции напрямую связан с типами подлежащих распределений характеристик. Более того, отсутствие значимости корреляции может быть обоснованным поводом не строить ребро между соответствующими узлами. 

2. Таким образом, мы получили корреляционный граф на момент времени $T$. 
   
3. Следующий этап - построение аналогичного графа для следующего момента времени $T_{+1}$: временное окно из 20 временных точек переезжает на один временный промежуток вперед и повторяется полная последовательность 3. - 5. Создается новый граф для момента времени $T_{+1}$ и так далее. 
